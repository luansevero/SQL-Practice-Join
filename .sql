...
--Questão Bônus:
SELECT s.id,s.name as school,c.name as course, comp.name as company, r.name as role FROM companies comp JOIN jobs j ON comp.id = j."companyId" JOIN roles r ON j."roleId" = r.id JOIN applicants a ON j.id = a."jobId" JOIN educations e ON a."userId" = e."userId" JOIN schools s ON e."schoolId" = s.id JOIN courses c ON e."courseId" = c.id WHERE comp.id = 10 AND j.active = true;
